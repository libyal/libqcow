...
def pyqcow_test_single_open_close_file_object_with_dereference( filename, mode ):
	file_object = open( filename, mode )
	qcow_file = pyqcow.file()
	qcow_file.open_file_object( file_object, mode )
	del file_object
...
def main( argc, argv ):
	...
	if argc < 2:
		print "Usage: pyqcow_test_open_close.py filename\n"
		return 1

	print "Testing single open close of: %s with access: read\t" %( sys.argv[ 1 ] ),

	try:
		pyqcow_test_single_open_close_file( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	result = 1
	try:
		pyqcow_test_single_open_close_file( None, "r" )
	...
	result = 1
	try:
		pyqcow_test_single_open_close_file( argv[ 1 ], "w" )
	except ValueError, exception:
		if exception.message == "pyqcow_file_open: unsupported mode: w.":
			result = 0
	except:
		pass
	...
	try:
		pyqcow_test_multi_open_close_file( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	try:
		pyqcow_test_single_open_close_file_object( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	try:
		pyqcow_test_single_open_close_file_object_with_dereference( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	try:
		pyqcow_test_multi_open_close_file_object( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
