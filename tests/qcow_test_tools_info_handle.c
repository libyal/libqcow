...
#include "../qcowtools/info_handle.h"

/* Tests the info_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int qcow_test_tools_info_handle_initialize(
     void )
{
 ...
 * Returns 1 if successful or 0 if not
 */
int qcow_test_tools_info_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	QCOW_TEST_RUN(
	 ...
	 qcow_test_tools_info_handle_initialize );

	QCOW_TEST_RUN(
	 "info_handle_free",
	 qcow_test_tools_info_handle_free );
