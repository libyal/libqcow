...
#define QCOW_TEST_READ_NUMBER_OF_THREADS	4
 ...
 */
int qcow_test_read_callback_function(
     libqcow_file_t *file,
     void *arguments QCOW_TEST_ATTRIBUTE_UNUSED )
{
	uint8_t buffer[ QCOW_TEST_READ_BUFFER_SIZE ];

	libcerror_error_t *error = NULL;
	static char *function    = "qcow_test_read_callback_function";
     ...
     int number_of_threads )
{
	...
	static char *function                  = "qcow_test_read_from_file_multi_thread";
	...
	if( number_of_threads > 1 )
	{
		...
		if( libcthreads_thread_pool_create(
		     ...
		     number_of_threads,
		     number_of_iterations,
		     (int (*)(intptr_t *, void *)) &qcow_test_read_callback_function,
		     NULL,
		     &error ) != 1 )
		{
			libcerror_error_set(
