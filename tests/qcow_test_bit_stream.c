...
#include "qcow_test_libcnotify.h"
#include "qcow_test_macros.h"
#include "qcow_test_memory.h"
#include "qcow_test_unused.h"

#include "../libqcow/libqcow_bit_stream.h"

/* Define to make qcow_test_bit_stream generate verbose output
#define QCOW_TEST_BIT_STREAM_VERBOSE
 */

uint8_t qcow_test_bit_stream_data[ 16 ] = {
...
/* Tests the libqcow_bit_stream_initialize function
 * Returns 1 if successful or 0 if not
 */
int qcow_test_bit_stream_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libqcow_bit_stream_initialize(
	          &bit_stream,
	          qcow_test_bit_stream_data,
	          16,
	          0,
	...
	result = libqcow_bit_stream_free(
	          &bit_stream,
	          &error );
	...
	bit_stream = (libqcow_bit_stream_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libqcow_bit_stream_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libqcow_bit_stream_initialize with memset failing
		 */
...
/* Tests the libqcow_bit_stream_free function
 * Returns 1 if successful or 0 if not
 */
int qcow_test_bit_stream_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
/* Tests the libqcow_bit_stream_get_value function
 * Returns 1 if successful or 0 if not
 */
int qcow_test_bit_stream_get_value(
     void )
{
	libcerror_error_t *error         = NULL;
	libqcow_bit_stream_t *bit_stream = NULL;
	...
	/* Test regular cases
	 */
	result = libqcow_bit_stream_get_value(
	          bit_stream,
	          0,
	          &value_32bit,
	          &error );
...
#endif
{
...
#if defined( HAVE_DEBUG_OUTPUT ) && defined( QCOW_TEST_BIT_STREAM_VERBOSE )
	...
	QCOW_TEST_RUN(
	 "libqcow_bit_stream_initialize",
	 qcow_test_bit_stream_initialize );

	QCOW_TEST_RUN(
	 "libqcow_bit_stream_free",
	 qcow_test_bit_stream_free );

	QCOW_TEST_RUN(
	 "libqcow_bit_stream_get_value",
	 qcow_test_bit_stream_get_value );
