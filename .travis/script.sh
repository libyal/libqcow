...
then
	docker run -t -v "${PWD}:/libqcow" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libqcow && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libqcow;
