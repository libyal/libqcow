...
const char *libqcow_get_version(
             void )
{
	return( (const char *) LIBQCOW_VERSION_STRING );
}
...
int libqcow_get_access_flags_read(
     void )
{
	return( (int) LIBQCOW_ACCESS_FLAG_READ );
}
...
int libqcow_get_access_flags_read_write(
     void )
{
	return( (int) ( LIBQCOW_ACCESS_FLAG_READ | LIBQCOW_ACCESS_FLAG_WRITE ) );
}
...
int libqcow_get_access_flags_write(
     void )
{
	return( (int) LIBQCOW_ACCESS_FLAG_WRITE );
}
...
{
	static char *function = "libqcow_get_codepage";
...
int libqcow_set_codepage(
     ...
     libcerror_error_t **error )
{
	static char *function = "libqcow_set_codepage";
...
/* Determines if a file is a QCOW file (check for the QCOW file signature)
     ...
     libcerror_error_t **error )
{
	...
	static char *function            = "libqcow_check_file_signature";
...
{
	libbfio_handle_t *file_io_handle = NULL;
	static char *function            = "libqcow_check_file_signature_wide";
	size_t filename_length           = 0;
	int result                       = 0;
...
/* Determines if a file is a QCOW file (check for the QCOW file signature) using a Basic File IO (bfio) handle
 * Returns 1 if true, 0 if not or -1 on error
 */
